...
class HarshTester:
    ...
    # -------------------------
    # Core backtest with cash+positions
    # -------------------------
    def backtest_signals(self, entries, exits, data=None):
        ...
        if len(daily_returns) > 1:
            ...
            sharpe = mu / sd * np.sqrt(252) if sd > 1e-8 else 0.0
            neg = daily_returns[daily_returns < 0]
            dsd = np.std(neg) if len(neg) > 0 else sd
            sortino = mu / dsd * np.sqrt(252) if dsd > 1e-15 else 0.0
        else:
            sharpe = 0.0
            sortino = 0.0
    ...
    # -------------------------
    # Test 1: Grid Search (supports data override)
    # -------------------------
    def stress_test(self, strategy_code: str, param_grid: dict, data=None, verbose: bool = False) -> pd.DataFrame:
    ...
    def walk_forward_test(self, strategy_code: str, param_grid: dict, train_years: int = 2, test_years: int = 1) -> pd.DataFrame:
        ...
        for start_idx in range(0, max(1, len(dates) - train_days - test_days), test_days):
            ...
            for combo in product(*param_grid.values()):
                ...
                e_test, x_test = strat_test.generate_signals()
                if isinstance(e_test, pd.Series):
                    e_test = e_test.to_frame()
                if isinstance(x_test, pd.Series):
                    x_test = x_test.to_frame()
                test_stats = self.backtest_signals(e_test, x_test, data=test_data)
    ...
    # -------------------------
    # Test 3–10 (existing APIs), then 11–20 per spec
    # -------------------------
        ...
        for result in base_results:
            ...
            if len(returns) > 10 and np.std(returns) > 1e-8:
                ...
                for _ in range(n_sims):
                    ...
                    mu = np.mean(resampled_ret)
                    sd = np.std(resampled_ret)
                    mc_sharpe = mu / sd * np.sqrt(252) if sd > 1e-8 else 0
                    neg = resampled_ret[resampled_ret < 0]
                    dsd = np.std(neg) if len(neg) > 0 else sd
                    mc_sortino = mu / dsd * np.sqrt(252) if dsd > 1e-15 else 0.0
                ...
                result_dict.update({
                    ...
                    'base_sortino': float(base['sortino']),
                    'mc_sharpe_5pct': float(base['sharpe']),
                    'mc_sharpe_mean': float(base['sharpe']),
                    'mc_sharpe_95pct': float(base['sharpe']),
                    'mc_sortino_5pct': float(base['sortino']),
                    'mc_sortino_mean': float(base['sortino']),
                    'mc_sortino_95pct': float(base['sortino']),
                    'base_max_dd': float(base['max_dd']),
                    'mc_dd_5pct': float(base['max_dd']),
                    'mc_dd_mean': float(base['max_dd']),
                    'mc_dd_95pct': float(base['max_dd'])
                })
            ...
            for param_name in param_grid.keys():
                ...
                try:
                    ...
                    for delta in [-perturbation, 0, perturbation]:
                        new_val = original_value * (1 + delta)
    ...
    def kelly_criterion_test(self, strategy_code: str, param_grid: dict) -> pd.DataFrame:
        ...
        avg_win = base_results[base_results['total_return'] > 0]['total_return'].mean() if num_wins > 0 else 0.01
        avg_loss = abs(base_results[base_results['total_return'] < 0]['total_return'].mean()) if num_losses > 0 else 0.01
        kelly = (win_rate * avg_win - (1 - win_rate) * avg_loss) / avg_loss if abs(avg_loss) > 1e-6 else 0.0
        kelly = max(0, min(kelly, 1))
        status = "Not tradeable: Negative edge" if kelly == 0.0 else ("Very risky: Low edge" if kelly < 0.1 else "Tradeable")
        return pd.DataFrame({
    ...
    def _coerce_intlike_params(self, params: dict) -> dict:
        window_keys = {'fast', 'slow', 'lookback', 'period', 'fast_period', 'slow_period', 'signal_period'}
        out = {}
        for k, v in params.items():
            if k in window_keys:
                try:
                    out[k] = int(max(1, round(float(v))))
                except:
                    out[k] = v
            else:
                out[k] = v
    ...
    # =========================
    # Test 11: Conditional EV decomposition
    # =========================
    def test_conditional_decomposition(self, strategy_code, param_grid):
        ...
        for combo in product(*param_grid.values()):
            ...
            if len(trades) == 0:
                results.append({
                    'param_set': params, 'total_trades': 0, 'win_rate': 0.0,
                    'avg_win': 0.0, 'avg_loss': 0.0, 'ev_per_trade': 0.0,
                    'ev_contribution_win': 0.0, 'ev_contribution_loss': 0.0
                })
                continue
    ...
    # =========================
    # Test 12: Time Slice Stability (EV, win rate, avg win/loss, max_dd + CV + decay flag)
    # =========================
    def test_time_slice_stability(self, strategy_code, param_grid, n_slices=4):
        ...
        for combo in product(*param_grid.values()):
            params = dict(zip(keys, combo))
            slice_evs, slice_winrates, slice_avgwins, slice_avglosses, slice_dds = [], [], [], [], []
            for s in range(n_slices):
                ...
                wins = rets[rets > 0]
                losses = rets[rets <= 0]
                wr = float(len(wins) / len(rets)) if len(rets) > 0 else 0.0
                aw = float(np.mean(wins)) if len(wins) > 0 else 0.0
                al = float(np.mean(losses)) if len(losses) > 0 else 0.0
                eq = self._equity_from_trades(rets, initial_equity=self.init_cash, f=1.0)
                dd = self._max_drawdown_from_equity(eq)
                slice_evs.append(ev); slice_winrates.append(wr); slice_avgwins.append(aw); slice_avglosses.append(al); slice_dds.append(dd)
            ...
            decay_flag = bool(len(ev_arr) >= 2 and ev_arr[-1] < 0.5 * ev_arr[0])
            row = {'param_set': params, 'ev_cv': ev_cv, 'ev_stability_score': float(max(0.0, 1.0 - min(1.0, ev_cv))), 'ev_decay_flag': decay_flag}
            for s in range(n_slices):
                row[f'slice_{s+1}_ev'] = float(ev_arr[s]) if s < len(ev_arr) else 0.0
                row[f'slice_{s+1}_win_rate'] = float(slice_winrates[s]) if s < len(slice_winrates) else 0.0
                row[f'slice_{s+1}_avg_win'] = float(slice_avgwins[s]) if s < len(slice_avgwins) else 0.0
                row[f'slice_{s+1}_avg_loss'] = float(slice_avglosses[s]) if s < len(slice_avglosses) else 0.0
                row[f'slice_{s+1}_max_dd'] = float(slice_dds[s]) if s < len(slice_dds) else 0.0
            results.append(row)
        return pd.DataFrame(results)

    # =========================
    # Test 13: Regime Dependency (EV, win_rate, max_dd per tercile)
    # =========================
    def test_regime_dependency(self, strategy_code, param_grid, regime_var='volatility'):
        ...
        var_series = self._realized_vol(self.data, window=20) if regime_var == 'volatility' else self.data.pct_change().abs()
        var_flat = var_series.values.flatten()
        var_flat = var_flat[np.isfinite(var_flat)]
        if len(var_flat) < 50:
            return pd.DataFrame()
        terciles = np.percentile(var_flat, [33.33, 66.67])
        ...
        for combo in product(*param_grid.values()):
            ...
            buckets = {0: [], 1: [], 2: []}
            for t in trades:
                i = t['entry_idx']; j = t['asset']
                v = float(var_series.iloc[i, j])
                if v <= terciles[0]: buckets[0].append(t['ret'])
                elif v <= terciles[1]: buckets[1].append(t['ret'])
                else: buckets[2].append(t['ret'])

            row = {'param_set': params}
            evs = []
            for r in [0, 1, 2]:
                rs = np.array(buckets[r], dtype=float)
                ev = float(np.mean(rs)) if len(rs) > 0 else 0.0
                wr = float(np.mean(rs > 0)) if len(rs) > 0 else 0.0
                eq = self._equity_from_trades(rs, initial_equity=self.init_cash, f=1.0)
                dd = self._max_drawdown_from_equity(eq)
                row[f'regime_{r+1}_name'] = ['low', 'med', 'high'][r]
                row[f'regime_{r+1}_ev'] = ev
                row[f'regime_{r+1}_win_rate'] = wr
                row[f'regime_{r+1}_dd'] = dd
                evs.append(ev)
            row['regime_consistency_score'] = float(min(evs) / max(evs)) if max(evs) != 0 else 0.0
            results.append(row)
        return pd.DataFrame(results)

    # =========================
    # Test 14: Win/Loss Distribution Stability (KS divergence, with counts)
    # =========================
    def test_win_loss_distribution_stability(self, strategy_code, param_grid, n_slices=2, min_samples=5):
        StrategyClass = self.load_strategy_class(strategy_code)
        keys = list(param_grid.keys())
        n = len(self.data)
        if n_slices < 2 or n < n_slices * 40:
        ...
        for combo in product(*param_grid.values()):
            ...
            w0, wN = slice_wins[0], slice_wins[-1]
            l0, lN = slice_losses[0], slice_losses[-1]
            have_wins = (len(w0) >= min_samples and len(wN) >= min_samples)
            have_losses = (len(l0) >= min_samples and len(lN) >= min_samples)
            winner_div = self._ks_divergence(w0, wN) if have_wins else float('nan')
            loser_div = self._ks_divergence(l0, lN) if have_losses else float('nan')
            combined = float(1.0 - min(1.0, 0.5 * (winner_div + loser_div))) if np.isfinite(winner_div) and np.isfinite(loser_div) else float('nan')
            results.append({
                'param_set': params,
                'wins_slice1': int(len(w0)),
                'wins_sliceN': int(len(wN)),
                'losses_slice1': int(len(l0)),
                'losses_sliceN': int(len(lN)),
    ...
    # =========================
    # Test 15: Sized Drawdown Simulation (Kelly, fixed 2%, vol-adjusted)
    # =========================
    def test_sized_drawdown_simulation(self, strategy_code, param_grid, kelly_fraction=0.25, initial_capital=100000, target_trade_vol=0.01):
        StrategyClass = self.load_strategy_class(strategy_code)
        keys = list(param_grid.keys())
        results = []
        ...
        for combo in product(*param_grid.values()):
            ...
            f_vol = float(target_trade_vol / sig) if sig > 1e-8 else 0.0
            f_vol = max(0.0, min(f_vol, 1.0))
            eq_vol = self._equity_from_trades(rets, initial_equity=initial_capital, f=f_vol)
            dd_vol = self._max_drawdown_from_equity(eq_vol)
            final_vol = float(eq_vol[-1]) if len(eq_vol) > 0 else float(initial_capital)

            rob = float(max(0.0, 1.0 - max(abs(dd_kelly), abs(dd_fixed), abs(dd_vol)) / 0.6))
            results.append({
                'param_set': params,
                'kelly_frac': f_kelly,
                ...
                'vol_adj_frac': f_vol,
                'max_dd_vol_adj': dd_vol,
                'final_equity_vol_adj': final_vol,
                'sizing_robustness_score': rob
            })
        return pd.DataFrame(results)

    # =========================
    # Test 16: Parameter Robustness (local hypercube around best)
    # =========================
    def test_parameter_robustness(self, strategy_code, param_grid):
        ...
        for pname, pvals in param_grid.items():
            base_val = float(best_params[pname])
            if isinstance(pvals[0], int):
                neighbor_grid[pname] = [int(max(1, round(base_val * 0.9))), int(max(1, round(base_val))), int(max(1, round(base_val * 1.1)))]
            else:
                neighbor_grid[pname] = [base_val * 0.9, base_val, base_val * 1.1]

        neighbors = [dict(zip(neighbor_grid.keys(), combo)) for combo in product(*neighbor_grid.values())]

        evs, drops = [], []
        for nb in neighbors:
            nb_df = self.stress_test(strategy_code, {k: [v] for k, v in nb.items()})
            if nb_df.empty:
                continue
            ev = float(nb_df['total_return'].mean())
            evs.append(ev)
            drops.append(float(best_ev - ev) if ev < best_ev else 0.0)

        threshold = best_ev * 0.9
        robustness_score = float(np.mean([1.0 if ev >= threshold else 0.0 for ev in evs])) if len(evs) > 0 else 0.0
        sensitivity = float(np.mean(drops)) if len(drops) > 0 else float('nan')

        return pd.DataFrame([{
            'param_set': best_params,
            'best_ev': best_ev,
            'neighbors_evaluated': len(evs),
            'robustness_score': robustness_score,
            'avg_sensitivity_drop': sensitivity
        }])

    # =========================
    # Test 17: Market Regime Drift (rolling beta; bull vs bear Sharpe)
    # =========================
    def test_market_regime_drift(self, strategy_code, param_grid):
        ...
        for combo in product(*param_grid.values()):
            params = dict(zip(list(param_grid.keys()), combo))
            strat = StrategyClass(self.data, params)
            entries, exits = strat.generate_signals()
            stats = self.backtest_signals(entries, exits)
    ...
    # =========================
    # Test 18: Costs impact (EV, win_rate, Sharpe, Sortino with/without costs)
    # =========================
    def test_slippage_impact(self, strategy_code, param_grid, slippage_bps=1, commission_bps=0.5):
        ...
        for combo in product(*param_grid.values()):
            ...
            ev_no = float(np.mean(rets)) if len(rets) else 0.0
            wr_no = float(np.mean(rets > 0)) if len(rets) else 0.0
            sharpe_no = self._sharpe_from_returns(rets)
            neg_no = rets[rets < 0]; dsd_no = np.std(neg_no) if len(neg_no) > 0 else np.std(rets)
            sortino_no = float(np.mean(rets) / dsd_no * np.sqrt(252)) if dsd_no > 1e-15 else 0.0

            rets_c = rets - per_trade_cost
            ev_wc = float(np.mean(rets_c)) if len(rets_c) else 0.0
            wr_wc = float(np.mean(rets_c > 0)) if len(rets_c) else 0.0
            sharpe_wc = self._sharpe_from_returns(rets_c)
            neg_wc = rets_c[rets_c < 0]; dsd_wc = np.std(neg_wc) if len(neg_wc) > 0 else np.std(rets_c)
            sortino_wc = float(np.mean(rets_c) / dsd_wc * np.sqrt(252)) if dsd_wc > 1e-15 else 0.0

            degr = float((ev_no - ev_wc) / abs(ev_no)) if abs(ev_no) > 1e-12 else float('inf')
            survives = bool(sharpe_wc >= 0.8 * sharpe_no and ev_wc > 0)

            results.append({
                'param_set': params,
                'ev_no_cost': ev_no, 'win_rate_no_cost': wr_no, 'sharpe_no_cost': sharpe_no, 'sortino_no_cost': sortino_no,
                'ev_with_cost': ev_wc, 'win_rate_with_cost': wr_wc, 'sharpe_with_cost': sharpe_wc, 'sortino_with_cost': sortino_wc,
    ...
    # =========================
    # Test 19: Losing streaks & DD severity
    # =========================
    ...
    # =========================
    # Test 20: Out-of-Sample Walk-Forward Validation (train-select, test-evaluate)
    # =========================
    def test_walk_forward_validation(self, strategy_code, param_grid, train_pct=0.7, step_size=0.1):
        ...
        for start in range(0, n - train_len - step + 1, step):
            ...
            # Select best params on TRAIN slice only (true OOS selection) [3]
            base_train = self.stress_test(strategy_code, param_grid, data=train_data, verbose=False)
            if base_train.empty:
                continue
            best_row = base_train.loc[base_train['sharpe'].idxmax()]
            best_params = {k: best_row[k] for k in param_grid.keys()}
            best_params = self._coerce_intlike_params(best_params)
            ...
            # Overfitting ratio: train/test; red flag if ratio > 2.0 (50%+ test drop) [3]
            if np.isfinite(train_sharpe) and np.isfinite(test_sharpe) and test_sharpe > 1e-8:
                overfit_ratio = float(train_sharpe / test_sharpe)
                delta_sharpe_pct = float(1.0 - (test_sharpe / train_sharpe))
            else:
                overfit_ratio = float('nan')
                delta_sharpe_pct = float('nan')

            is_overfit = bool(overfit_ratio > 2.0) if np.isfinite(overfit_ratio) else False
